...
export class DepositAggregator extends SmartContract {
    ...
    public aggregate(
        ...
        if (level == 0n) {
            ...
            const hashData0 = DepositAggregator.hashAggregatedDepositData(level, ancestorTx0.hashData, ancestorTx1.hashData)
            const hashData1 = DepositAggregator.hashAggregatedDepositData(level, ancestorTx2.hashData, ancestorTx3.hashData)
        ...
        // output.
        const newHash = DepositAggregator.hashAggregatedDepositData(level + 1n, prevTx0.hashData, prevTx1.hashData)
    ...
    @method()
    static hashDepositData(level: bigint, depositAddress: ByteString, depositAmt: bigint): Sha256 {
        return hash256(
            MerklePath.levelToByteString(level) + sha256(depositAddress + GeneralUtils.padAmt(depositAmt))
        )
    }

    @method()
    static hashAggregatedDepositData(level: bigint, left: Sha256, right: Sha256): Sha256 {
        return hash256(
            MerklePath.levelToByteString(level) + left + right
        )
    }
