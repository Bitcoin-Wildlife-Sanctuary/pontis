import { ByteString, SmartContract, UTXO } from 'scrypt-ts'
...
export abstract class Covenant<StateT = undefined> {
  ...
  constructor(
    ...
    this.address = p2trLockingScriptToAddr(
      this.lockingScript.toHex(),
      options.network
    )
    this.asmVersion = Covenant.calculateAsmVersion(
      subContracts.map(
        (c) =>
          (c.contract.constructor as typeof SmartContract).getArtifact().md5
      )
    )

    if (this.asmVersion !== options.lockedAsmVersion) {
      throw new Error(
        `Locked ASM version mismatch: current(${this.asmVersion}) vs locked(${options.lockedAsmVersion})`
      )
    }
  }
  ...
  get stateHash(): ByteString {
    return this.state ? this.serializedState() : ''
  }
