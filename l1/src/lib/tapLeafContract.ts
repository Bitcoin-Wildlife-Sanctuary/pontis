...
export class TapLeafSmartContract {
  ...
  contractCallToWitness(method: string, args: any[]): Buffer[] {
    ...
    const unlockingScript = (this.contract as any).delegateInstance[method](
      ...args_
    ).unlockingScript
  ...
  witnessToContractCallArgs(witness: Buffer[], method: string): Arguments {
    const abiCoder: ABICoder = (this.contract as any).getDelegateClazz()
      .abiCoder
    ...
    if (abiEntity.params.length > argsWitness.length) {
      throw new Error(
        `Unexpected number of witness elements, got ${
          witness.length - 2
        }, expected at least ${abiEntity.params.length}`
      )
    }
    let unlockingScriptHex = Buffer.from(
      bitcoinjs.script.compile(argsWitness)
    ).toString('hex')
    if (abiEntity.index) {
      unlockingScriptHex =
        unlockingScriptHex + int2ByteString(BigInt(abiEntity.index))
    }
    try {
