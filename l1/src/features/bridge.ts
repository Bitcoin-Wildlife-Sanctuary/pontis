...
import { BridgeCovenant, DepositAggregatorCovenant, stateToBatchID, TraceableBridgeUtxo, TraceableDepositAggregatorUtxo, TracedBridge, TracedDepositAggregator, WithdrawalExpanderCovenant, WithdrawalExpanderState } from "../covenants";
...
import { BatchMerkleTree, BridgeMerkle, Withdrawal, WithdrawalMerkle } from "../util/merkleUtils";
...
export async function finalizeL1Deposit(
    ...
    const replacedNodeIndex = oldState.merkleTree.indexOf(BridgeMerkle.EMPTY_BATCH_ID)
    ...
    const tracedDepositAggregator = await DepositAggregatorCovenant.backtrace(depositAggregatorUtxo, chainProvider)

    const newState = BridgeCovenant.createEmptyState(tracedDepositAggregator.covenant.bridgeSPK)
    newState.merkleTree = oldState.merkleTree.map(
        (batchId, index) => index === replacedNodeIndex ? finalizedBatchId : batchId
    ) as BatchMerkleTree;
...
export async function finalizeL2Deposit(
    ...
    const replacedNodeIndex = bridgeUtxo.state.merkleTree.indexOf(finalizedBatchId);
    if (replacedNodeIndex < 0) {
    ...
    const tracedBridge = await BridgeCovenant.backtrace(bridgeUtxo, chainProvider)
    const newState = BridgeCovenant.createEmptyState(oldState.depositAggregatorSPK)
    newState.merkleTree = oldState.merkleTree.map(
        (batchId) => batchId === finalizedBatchId ? BridgeMerkle.EMPTY_BATCH_ID : batchId
    ) as BatchMerkleTree;
    newState.batchesRoot = BridgeMerkle.calcMerkleRoot(newState.merkleTree);
    newState.depositAggregatorSPK = oldState.depositAggregatorSPK;
...
export async function createWithdrawalExpander(
    ...
    withdrawals.forEach(withdrawal => WithdrawalMerkle.checkWithdrawalValid(withdrawal))
    const sumAmt = withdrawals.reduce((acc, withdrawal) => acc + withdrawal.amt, 0n);
    ...
    const outputWithdrawalState: WithdrawalExpanderState = withdrawalLevels.length === 1 ?
...
function buildCreateWithdrawalTx(
    ...
    const createWithdrawalTx = new ExtPsbt()
        ...
        .addCovenantOutput(outputBridgeCovenant, Number(bridgeUtxo.utxo.satoshis - Number(sumAmt)))
...
function buildFinalizeL1DepositTx(
    ...
    const finalizeL1Tx = new ExtPsbt()
        ...
        .addFeeInputs([feeUtxo])
        .addCovenantOutput(outputBridgeCovenant, bridgeUtxo.utxo.satoshis + depositAggregatorUtxo.utxo.satoshis)
        .addStateOutput(outputBridgeCovenant)
        .change(changeAddress, feeRate, estimatedVSize)
...
function buildDepolyBridgeTx(
    ...
    if (feeUtxo.satoshis < Postage.BRIDGE_POSTAGE + feeRate * (estimatedVSize || 1)) {
        throw new Error('fee utxo is not enough')
    }
