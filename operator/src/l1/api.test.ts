...
} from './deps/offchainDataProvider';
...
const operatorSigner = new DefaultSigner(
  ECPair.fromWIF('L2Lnz6FoeWLdEdG7Kk8JnBr1Vb2hdcvPNR1BkkfvVA9Piax83L5U')
);
...
describe('test l1 api', () => {
  ...
  beforeEach(async () => {
    ...
    l1Provider.setListUtxosByRawTxs([
      await defaultChainProvider.getRawTransaction(txid),
    ]);
  });
  ...
  it('api.getTransactionStatus should return status', async () => {
    ...
    const status = await api.getL1TransactionStatus(
      l1Provider,
      '8095bd333ba059defa5f28a4477f7799322a3792806b66bze32f2423fe82763a7'
    );
  ...
  it('api.createBridgeContractIfNotExists should not create new bridge contract when bridge contract exists', async () => {
    ...
    localL1Provider.setListUtxosByRawTxs([
      await defaultChainProvider.getRawTransaction(txid),
    ]);
  ...
  describe('test for listDeposits', () => {
    it('api.listDeposits should return deposits when utxo and offchain data are both have same deposit', async () => {
      ...
      deposits.forEach((d) => {
        if (d.origin.status === 'MINED') {
          d.origin.blockNumber = 1;
        }
      });
  ...
  async function createDeposits(depositCount: number, amountList?: bigint[]) {
    const deposits = [];
    ...
    for (let i = 0; i < depositCount; i++) {
      deposits.push(
        await api.createDeposit(
          ...
          defaultFeeRate,
          operatorSigner,
          l2Addresses[i % l2Addresses.length],
          amountList[i]
        )
      );
    }
