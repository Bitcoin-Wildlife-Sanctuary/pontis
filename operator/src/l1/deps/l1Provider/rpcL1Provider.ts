...
export class RPCL1Provider extends RPCChainProvider implements L1Provider {
  async getCurrentBlockNumber(): Promise<number> {
    const data = await btcRpc.rpc_getblockchaininfo(
      this.url,
      this.username,
      this.password
    );
  ...
  async listUtxos(
    ...
    const data = await btcRpc.rpc_listunspent(
      this.url,
      this.username,
      this.password,
      this.walletName,
    ...
    const utxos = data.map((utxo: any) => ({
      ...
      satoshis: new Decimal(utxo.amount).mul(new Decimal(100000000)).toNumber(),
      script: utxo.scriptPubKey,
      blockNumber:
        utxo.confirmations > 0
          ? blockNumber + 1 - utxo.confirmations
          : UNCONFIRMED_BLOCK_NUMBER,
    }));
    return utxos.filter(
      (utxo: Utxo) =>
  ...
  async getTransactionStatus(txid: string): Promise<L1TxStatus> {
    const data = await btcRpc.rpc_gettransaction(
      this.url,
      this.username,
      this.password,
      this.walletName,
