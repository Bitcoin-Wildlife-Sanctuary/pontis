...
import {MempoolChainProvider, MempoolUtxoProvider, RPCChainProvider, RPCUtxoProvider, DefaultSigner, SupportedNetwork} from 'l1'
...
export const l1Network = process.env.L1_NETWORK! as SupportedNetwork;
{
  ...
  if (l1Network !== 'fractal-mainnet' && l1Network !== 'fractal-testnet' && l1Network !== 'btc-signet') {
...
}
{
  ...
  if (rpcConfig.host) {
    if (!rpcConfig.user || !rpcConfig.password || !rpcConfig.wallet || !rpcConfig.network) {
      throw new Error('RPC_USER, RPC_PASSWORD, RPC_WALLET, and L1_RPC_NETWORK must be set if RPC_HOST is set');
    }
    if (rpcConfig.network !== l1Network) { 
      throw new Error(`RPC_NETWORK ${rpcConfig.network} does not match L1_NETWORK ${l1Network}`);
    }
  }
}
...
export function createUtxoProvider() {
  if (rpcConfig.host) {
    return new RPCUtxoProvider(rpcConfig.host, rpcConfig.user, rpcConfig.password, rpcConfig.wallet);
  }
...
export function createChainProvider() {
  if (rpcConfig.host) {
    return new RPCChainProvider(rpcConfig.host, rpcConfig.user, rpcConfig.password, rpcConfig.wallet);
  }
