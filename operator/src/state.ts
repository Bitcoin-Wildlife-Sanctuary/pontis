...
export async function applyChange(
  ...
  switch (change.type) {
    ...
    case 'deposits': {
      await handleDeposits(env, newState, change);
      break;
    }
    case 'l2tx': {
    ...
    case 'l1tx': {
      ...
      manageAggregation(env, newState);
      break;
    }
    case 'tick': {
...
async function handleDeposits(
  ...
  await manageAggregation(env, state);
}
